...
const index = memo((props) => {
    ...
    const resendMail = (user, token) => {
        ...
        console.log('esta aca??', object)
        axios.post('/password/resend', object)
            .then(response => {
                if (response.data.ok) {
                    buttonsSwalModalStyle.fire(
                        'Done!',
                        'An mail has been send to your email, please check it to change your password!',
                        'success'
                    )
                } else {
                    swal.fire(
    ...
    return (
        <div className="container-verification">
            ...
            <h3>And email has been send</h3>
            <p style={{ textAlign: 'center' }}> Please check your email</p>
